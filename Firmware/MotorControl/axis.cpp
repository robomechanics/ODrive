...
bool Axis::run_sensorless_control_loop() {
    run_control_loop([this](){
        ...
        // 0 is a placeholder for vel_filtered, only present in encoder
        if (!controller_.update(sensorless_estimator_.pll_pos_, sensorless_estimator_.vel_estimate_, 0, &current_setpoint))
...
bool Axis::run_closed_loop_control_loop() {
    ...
    run_control_loop([this](){
        // Note that all estimators are updated in the loop prefix in run_control_loop
        float current_setpoint;
        if (!controller_.update(encoder_.pos_estimate_, encoder_.vel_estimate_, encoder_.vel_filtered_, &current_setpoint))
